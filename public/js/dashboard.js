...
(function ($) {
  ...
  $(function () {
    ...
    $.get("https://inlupp-fa.azurewebsites.net/api/total-sales-chart", function (data) {
      if ($("#total-sales-chart").length) {
        ...
        var areaData = {
          ...
          datasets: [
            ...
            {
              data: parsedData["datasets"][1]["data"],
              backgroundColor: [
                'rgba(241, 83, 110, .0)'
              ],
              borderColor: [
    ...
    $.get("https://inlupp-fa.azurewebsites.net/api/total-users", function (data) {
      if ($("#users-chart").length) {

        let parsedData = JSON.parse(data)

        var areaData = {
          labels: parsedData["dataset"]["labels"],
          datasets: [{
            data: parsedData["dataset"]["data"],
            backgroundColor: [
              '#e0fff4'
            ],
            borderWidth: 2,
    ...
    $.get("https://inlupp-fa.azurewebsites.net/api/total-projects", function (data) {
    ...
    $.get("https://inlupp-fa.azurewebsites.net/api/downloads", function (data) {
      if ($('#offlineProgress').length) {
        ...
        bar.animate(parsedData[0]["circleValue"]); // Number from 0.0 to 1.0
    ...
    $.get("https://inlupp-fa.azurewebsites.net/api/downloads", function (data) {
      if ($('#onlineProgress').length) {
        ...
        bar.text.style.fontSize = '1rem';
        bar.animate(parsedData[1]["circleValue"]); // Number from 0.0 to 1.0
      }
    });
    ...
    $.get("https://inlupp-fa.azurewebsites.net/api/distribution", function (data) {
      if ($("#distribution-chart").length) {
        ...
        var areaOptions = {
          ...
          legendCallback: function (chart) {
            ...
            parsedData["cities"].forEach(city => {
              text.push('<div class="item"><div class="legend-label" style="border: 3px solid ' + chart.data.datasets[0].backgroundColor[index] + '"></div>');
              text.push('<p>' + city + '</p>');
              text.push('</div>');
              index++;
            });
        ...
        var distributionChartPlugins = {
          beforeDraw: function (chart) {
            ...
            var text = parsedData["procentage"] + "%",
    ...
    $.get("https://inlupp-fa.azurewebsites.net/api/sales-report", function (data) {
      if ($("#sale-report-chart").length) {
        ...
        var CurrentChart = new Chart(CurrentChartCanvas, {
          ...
          data: {
            labels: parsedData["labels"],
            datasets: [{
              label: parsedData["datasets"][0]["label"],
              data: parsedData["datasets"][0]["data"],
